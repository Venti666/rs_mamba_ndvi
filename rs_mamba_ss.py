...
class RSM_SS(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if self.use_ndvi:
            self.ndvi_conv = nn.Sequential(
                nn.Conv2d(1, 16, kernel_size=3, padding=1),  # 单独处理NDVI通道
                nn.ReLU(),
                nn.Conv2d(16, 1, kernel_size=1)     # 压缩通道数
            )
        # 原patch_embed输入通道调整为6（原5 + NDVI）

        _make_patch_embed = self._make_patch_embed_v2
        self.patch_embed = self._make_patch_embed_v2(6 if use_ndvi else 5, dims[0], patch_size, patch_norm, norm_layer)
        ...
        self.upsample_x4 = nn.Sequential(
            ...
            nn.BatchNorm2d(8),
            nn.ReLU(inplace=True),
            nn.UpsamplingBilinear2d(scale_factor=2)  # 最终上采样到输入图像尺寸
        )
    ...
    @staticmethod
    def _make_patch_embed_v2(in_chans=6, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm):
        assert patch_size == 4
...
if __name__ == "__main__":
    import torch
    from torchsummary import summary  # 需要安装torchsummary: pip install torchsummary
    ...
    # 打印模型结构
    print("\nModel Summary:")
    summary(model, input_size=(5, 256, 256), batch_size=1)  # 假设输入尺寸为256x256
    ...
    try:
        ...
        # 验证输出维度是否正确
        assert output.shape[1] == 20, f"Output channels should be 20, but got {output.shape[1]}"
